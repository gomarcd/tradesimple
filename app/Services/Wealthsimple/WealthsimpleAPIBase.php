...
namespace App\Services\Wealthsimple;

use App\Services\Wealthsimple\Exceptions\CurlException;
use App\Services\Wealthsimple\Exceptions\LoginFailedException;
use App\Services\Wealthsimple\Exceptions\ManualLoginRequired;
use App\Services\Wealthsimple\Exceptions\OTPRequiredException;
use App\Services\Wealthsimple\Exceptions\UnexpectedException;
use App\Services\Wealthsimple\Exceptions\WSApiException;
...
abstract class WealthsimpleAPIBase
{
    ...
    protected function doGraphQLQuery(string $query_name, array $variables, string $data_response_path, string $expect_type, ?callable $filter = NULL) {
        // Remove null variables to avoid sending explicit `null` values to Wealthsimple.
        ...
        \Log::debug('Outgoing GraphQL Query', [
        ...
        \Log::debug('Raw GraphQL Response Data', [
            'query_name' => $query_name,
            'variables' => $variables,
            'response' => $response,
        ...
        foreach (explode('.', $data_response_path) as $key) {
            if (!is_object($data)) {
                \Log::error('Unexpected data type - expected object, got something else', [
                    'current_key' => $key,
                    'current_data' => $data,
                ]);
                throw new WSApiException("GraphQL query failed: $query_name - unexpected data type at '$key'", 0, $response);
            }
    
            if (!property_exists($data, $key)) {
                \Log::error('Missing expected key in response', [
                    'key' => $key,
                    'current_data' => $data,
                ]);
                throw new WSApiException("GraphQL query failed: $query_name - missing key '$key'", 0, $response);
            }
        ...
        if (($expect_type === 'array' && !is_array($data)) || ($expect_type === 'object' && !is_object($data))) {
            \Log::error('Unexpected final data type', [
                'expected' => $expect_type,
                'actual' => gettype($data),
                'final_data' => $data,
            ]);
            throw new WSApiException("GraphQL query failed: $query_name - unexpected final data type", 0, $response);
        }
    
        if ($key === 'edges' && is_array($data)) {
